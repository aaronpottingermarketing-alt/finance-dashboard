...
export async function GET() {
      ...
      try {
        ...
        if (soRes.ok) {
          ...
          for (const so of soData.results ?? []) {
            ...
            bills.push({
              merchant_name: so.payee ?? so.reference ?? 'Standing Order',
      ...
      try {
        ...
        if (ddRes.ok) {
          ...
          for (const dd of ddData.results ?? []) {
            ...
            bills.push({
              merchant_name: dd.name ?? 'Direct Debit',
              day_of_month: dayOfMonth,
              monthly_pence: amountPence,
  ...
  // Deduplicate by merchant name (same payee can appear across accounts)
  if (bills.length > 0) {
  ...
  // Fallback: detect from transaction patterns
